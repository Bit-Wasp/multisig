...
class Multisig2of3 extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->form_validation->run('submit_3public_keys') == TRUE)
		{
			if(BitcoinLib::import_public_key($this->input->post('public_key1') == FALSE) ||
			   ...
			   BitcoinLib::import_public_key($this->input->post('public_key3') == FALSE)){
				...
				if($this->bitcoin_model->check_have_watch_address($data['multisig']['address']) == FALSE){
					...
					$this->bitcoin_model->add_watch_address(array(	'public_key1' => $data['pubkeys']['public_key1'],
											...
											'public_key3' => $data['pubkeys']['public_key3'],
	...
	public function pay3address()
	{
		...
		$sent = $this->bitcoin_model->load_sent_transactions($data['address']);
